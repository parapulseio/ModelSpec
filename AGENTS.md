...
- **CLI: `verify <file>`**: validates a plain JSON/YAML ModelSpec document against `modelspec.schema.export_json_schema()` with `jsonschema` (the `verify` extra) — independent of `extract`, so it also catches a hand-edited or third-party document. `export_json_schema()` wraps `ModelSpec.model_json_schema()` with `$schema`/`$id`/`$comment` (draft + versioned URI + "generated by modelspec X.Y.Z") so the schema file `modelspec schema` prints (and the release workflow publishes as `modelspec.schema.json`) is self-identifying to a third party. See [docs/cli.md](docs/cli.md).

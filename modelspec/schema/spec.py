...
class ModelSpec(_Model):
    """Unified, provenance-tracked specification of one LLM.

    Normalizes the heterogeneous metadata a model ships with — config.json,
    GGUF/safetensors headers, LICENSE files, tokenizer configs, mergekit
    recipes — into one type-safe schema covering architecture, parameters,
    tokenizer, license, quantization, and merge lineage, with per-field
    provenance and confidence. See https://github.com/parapulseio/ModelSpec.

    ``spec_version`` (below) versions the shape of *this document*; the schema
    file itself is generated by a specific ``modelspec`` tool release — see
    this JSON Schema's own ``$id`` / ``$comment`` for that.
    """
...
def export_json_schema() -> dict[str, Any]:
    """The JSON Schema for ``ModelSpec`` documents, annotated for standalone use.

    ``ModelSpec.model_json_schema()`` alone has no ``$schema`` / ``$id`` and no
    reference to which ``modelspec`` release generated it — fine for in-process
    use, but not for a schema file handed to a third party. Adds:
      - ``$schema``: the JSON Schema draft this targets (2020-12, matching what
        ``modelspec verify`` validates against and what Pydantic v2 emits).
      - ``$id``: a versioned URI so a schema downloaded from one release is
        traceable back to it.
      - ``$comment``: a human-readable "generated by modelspec X.Y.Z" note.

    Used by both ``modelspec schema`` and the release workflow's
    ``modelspec.schema.json`` artifact — the single source of truth so both
    stay identical.
    """
    from modelspec import __version__  # local import: avoids a module-load cycle

    schema = ModelSpec.model_json_schema()
    return {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "$id": f"https://github.com/parapulseio/ModelSpec/releases/download/v{__version__}/modelspec.schema.json",
        **schema,
        "$comment": f"Generated by modelspec {__version__}. https://github.com/parapulseio/ModelSpec",
    }
